...
**Notes:**
1. **Nearest is unreachable when perpendicular is on.** `_geometric_snaps` emits `nearest` only inside an `elif self.snap_nearest:` branch under `if self.snap_perpendicular`. Effectively a single roadmap item across all line-bearing rows — see §12 item 5.
2. `ConstructionLine` is **not** a `QGraphicsLineItem` subclass; it's a custom `QGraphicsItem` with `pt1`/`pt2` attributes. `_geometric_snaps` only fires its line branch for true `QGraphicsLineItem`. Result: construction lines have **no perpendicular, no nearest, and contribute no segments to phase 4 intersection scans**. Triple bug, single root cause.
