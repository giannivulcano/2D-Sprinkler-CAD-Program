...
- [ ] Bug: PDF interactive placement (via import dialog) loses vector geometry on save/reload — `_commit_place_import` bakes vectors into QGraphicsItems but `import_pdf` re-renders as raster. PDFs placed via import dialog show as washed-out raster after save/reload instead of crisp vectors. Fix requires serializing PDF vector geometry dicts into the Underlay record and reconstructing on load (same approach as DXF). `model_space.py:_commit_place_import`, `model_space.py:import_pdf`, `underlay.py` [type:Bug] [P1] [subject:CAD]
